...
> [!NOTE]
> 正式版请只从本仓库的 [Releases](https://github.com/Sakura-Lhy0409/M-Control-Releases/releases) 下载，并核对发布页提供的 SHA256。大多数用户选择普通安装版；免安装可选普通便携版。内嵌 Fixed Version Runtime 的安装版可随基线版本或 hotfix 提供，但 hotfix 仍不提供 Fixed Runtime 便携 ZIP。
...
- **无自动遥测**：硬件数据用于本机监控与控制，不自动上传。问题反馈由用户选择资料并确认提交，不扫描个人文件。
...
- **灯效与快捷键**：SteelSeries / Mystic Light 灯效、快捷键状态反馈和 OSD 提示；逐键控制按拓扑及 HID 通道开放。PRISM 实时效果由算法生成，不重放抓包帧。
- **屏幕校色**：LM 导入和匹配内屏的 True Color 配置生成；HDR 开启暂停 SDR 校色，活动内屏缺失时不写外接屏。

风扇“恢复启动曲线”使用本次首次写入前快照，不是 BIOS 出厂重置。GPU 模式位于“性能”页，按能力显示；立即生效可能走普通重启或 S5 关机自动唤醒。电池健康、上限和校准在“设置”页。

> [!IMPORTANT]
> 当前正式版 GPU 功能以只读遥测和受支持机型的 GPU 模式切换为主，不包含 GPU 超频、VF 曲线、GPU 锁频、游戏 FPS 检测或硬件实时监控叠加层。
...
- 程序启动时请求 Windows UAC 管理员授权；普通版使用系统 WebView2，Fixed 版使用内嵌 Runtime。便携版不会自动安装 PawnIO。
- 使用自研 MSI ACPI 客户端与 MOF 资源，不要求安装 MSI Center 或旧官方运行时 DLL；系统平台驱动与固件接口仍需可用。

**可信原则**

- 正式发布包提供 SHA256；校验不一致时不要运行。
- 核心功能本地执行；硬件数据不用于遥测，也不会主动上传。
- 与 MSI Center 等硬件控制软件同时写同一功能可能产生冲突。
- 版本检查读取固定更新清单及配置镜像；确认更新后下载并验证安装器。Fixed 安装版保持 Fixed 更新类型，诊断报告不会自动上传。
- 兼容性按设备和具体功能记录，不将整个系列视为全功能认证。NVIDIA 6xx 的私有遥测崩溃及校色延迟变换修复已随 v1.3.49 提供，受影响设备仍应复测。

## 文档与反馈

v1.4.3起可在 **设置 → 硬件诊断 → 提交 BUG 反馈** 直接提交问题、截图、视频和诊断包。支持草稿、进度、取消重试及可复制编号；资料保存至私有后台，无需网盘。详见[反馈使用说明](docs/FEEDBACK.md)和[v1.4.3 Hotfix更新说明](docs/releases/v1.4.3.md)。
